<template>
  <n-button
    ...
    @click="showCollectionCloneModal = true"
  >
    <n-card style="width: 200px; height: 200px" title="Clone Collection">
      <div>
  ...
  <n-modal
    v-model:show="showCollectionCloneModal"
  ...
  >
    <p style="display: flex; padding-bottom: 5px;">
      Create a new collection with the config of an existing collection, and
      copy all its contents to it as is. If multi-tenancy is enabled, same
      tenants and contents will be created.
    </p>
    <n-card>
      ...
      <n-divider />
      <div style="display: flex; gap: 16px">
        <p style="font-size: medium">Target Collection Name:</p>
        <n-input v-model:value="targetCollection" type="text" placeholder="" />
      </div>
      <n-divider />
      <div style="display: flex; gap: 16px">
    ...
    <n-card>
      <div v-if="!processStarted">
        <n-button type="info" :onclick="handleCloneCollection">
          Clone
        </n-button>
      </div>
      <div
...
const showCollectionCloneModal = ref<boolean>(false);
...
async function handleCloneCollection() {
  processStarted.value = true;

  const processId = await migrationService.cloneCollection(
